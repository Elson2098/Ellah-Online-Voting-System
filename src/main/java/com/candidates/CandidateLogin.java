	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (captchaDB.equals(vercode)) {
                ResultSet resultset = st.executeQuery("select * from tblcandidate where uname='" + uname + "' AND upass='" + pass + "'");
                if (resultset.next()) {
                    ...
                    response.sendRedirect("candidate-dashboard.jsp?_tokens='" + tokens + "'");
                } else {
